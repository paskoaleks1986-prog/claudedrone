...
| 5 | **d_perp** | дист ЦЕНТР→ближайшая followable-стена (GT, без конверсии). 🔒 LOCK a8ac4d0: standoff-петля конвертит сама `d_perp_beam = d_perp − sensor_ring_r(0.10)` перед сравнением с d*(beam). collision (idx11) — center. |
...
⚠️ **Геометрия дескриптора = ПОВЕРХНОСТЬ стены** (грань на стороне дрона = centerline − WALL_T/2), НЕ центральная линия box — ToF/коллизия видят поверхность (dev-log/48, фикс tof-parity 0.075м). Спавн = center→поверхность 0.50 (P0 x=1.425, P2 r=3.925). rl-env raycast тоже к поверхности/gz.
